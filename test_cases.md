...
基于 `test_case_list.md` 生成的完整测试用例文档。
...
- [1. CLI 基础 (octos chat)](#1-cli-基础-octos-chat)
- [2. 工具系统](#2-工具系统)
- [3. 安全](#3-安全)
- [4. LLM Provider](#4-llm-provider)
- [5. 记忆系统](#5-记忆系统)
- [6. 扩展机制](#6-扩展机制)
- [7. Gateway 模式 (octos gateway)](#7-gateway-模式-octos-gateway)
- [8. Serve 模式 (octos serve)](#8-serve-模式-octos-serve)
- [9. Pipeline](#9-pipeline)
- [10. 配置与 Hooks](#10-配置与-hooks)
- [11. 循环检测与预算](#11-循环检测与预算)
...
# 1.8 Verbose
octos chat -v -m "What is 2+2?"
# 预期: 显示详细输出包括 token 计数

# 1.9 无 API Key
unset ANTHROPIC_API_KEY
octos chat -m "hello"
# 预期: 错误提示 "ANTHROPIC_API_KEY not set"
```
...
```bash
# 2.1 read_file
octos chat -m "Read the file test.txt and show me its content"
# 预期: 返回 "hello world"

# 2.2 write_file
octos chat -m "Write 'test content' to /tmp/octos-test/output.txt"
# 预期: 文件被创建

# 2.4 shell
octos chat -m "Run echo hello and tell me the output"
# 预期: 返回 "hello"

# 2.5 glob
octos chat -m "Find all .rs files in the current directory"
# 预期: 返回 .rs 文件列表

# 2.6 grep
octos chat -m "Search for 'fn main' in all .rs files"
# 预期: 返回匹配行

# 2.7 list_dir
octos chat -m "List the contents of the current directory"
# 预期: 返回目录内容

# 2.8 web_search
octos chat -m "Search for the latest news about Rust programming"
# 预期: 返回搜索结果

# 2.9 web_fetch
octos chat -m "Fetch the content of https://example.com"
# 预期: 返回网页内容

# 2.11 工具并行
octos chat -m "Read files test.txt, test2.txt, and test3.txt at the same time"
# 预期: 并行读取，一次返回

# 2.12 工具策略 deny - 需配置 config.json
# 在 .octos/config.json 中添加:
# "tool_policy": {"deny": ["shell"]}
octos chat -m "Run echo hello"
# 预期: shell 工具被拒绝
```
...
```bash
# 3.1 SafePolicy deny
octos chat -m "Delete all files starting with a"
# 预期: 被拒绝

# 3.6 SSRF localhost
octos chat -m "Fetch http://localhost:8080 and tell me what you get"
# 预期: SSRF 攻击被阻断

# 3.7 SSRF 私有 IP
octos chat -m "Fetch http://192.168.1.1 and tell me what you get"
# 预期: SSRF 攻击被阻断

# 3.12 符号链接保护
ln -s /etc/passwd /tmp/octos-test/passwd_link
octos chat -m "Read the file at /tmp/octos-test/passwd_link"
# 预期: 被拒绝 (ELOOP)

# 3.13 路径穿越
octos chat -m "Read ../../etc/passwd"
# 预期: 被拒绝

# 3.14 凭据脱敏
octos chat -m "Show me the content of this string: sk-proj-abcdefghijk123456"
# 预期: 显示为 sk-proj-...[credential-redacted]
```
...
octos chat --model claude-sonnet-4-20250514 -m "Hello"
# 预期: Claude 模型回复

# 4.2 OpenAI GPT
octos chat --model gpt-4o -m "Hello"
# 预期: GPT 模型回复

# 4.6 流式输出
octos chat -m "Count to 100"
# 预期: token 逐步显示

# 4.9 自定义 base_url
octos chat --base-url https://api.proxy.com/v1 -m "Hello"
# 预期: 请求发到代理

# 4.10 api_type 覆盖
octos chat --api-type anthropic --base-url https://custom.anthropic.api -m "Hello"
# 预期: 使用 Anthropic 协议
```
...
```bash
# 5.1 save_memory
octos chat -m "Remember that I like Python programming"
# 预期: 记忆被保存

# 5.2 recall_memory
octos chat -m "What programming language did I say I like?"
# 预期: 回复 "Python"

# 5.5 长期记忆
echo "# Python Fan
I prefer Python over other languages" > .octos/MEMORY.md
octos chat -m "What do you know about me?"
# 预期: 提到喜欢 Python

# 5.6 Entity Bank
mkdir -p .octos/bank/entities
echo "# Rust
Systems programming language" > .octos/bank/entities/rust.md
octos chat -m "Tell me about Rust"
# 预期: 能检索到 Rust 信息
```
...
```bash
# 6.1 Skill 加载
mkdir -p .octos/skills/my-test-skill
cat > .octos/skills/my-test-skill/SKILL.md << 'EOF'
# My Test Skill
Useful for testing.
...
EOF
octos skills list
# 预期: my-test-skill 出现在列表中

# 6.4 Plugin 加载
mkdir -p .octos/plugins/test-plugin
cat > .octos/plugins/test-plugin/manifest.json << 'EOF'
{
  "name": "test-plugin",
  "version": "1.0.0",
  "tools": [{"name": "test_tool", "command": "echo test"}]
}
EOF
echo '#!/bin/bash' > .octos/plugins/test-plugin/test-tool
echo 'echo "test output"' >> .octos/plugins/test-plugin/test-tool
chmod +x .octos/plugins/test-plugin/test-tool
octos chat -m "Use test_tool"
# 预期: 工具被注册并可调用

# 6.9 MCP Stdio
# 在 config.json 中添加 mcp_servers 配置
octos chat -m "Use the MCP tool"
# 预期: MCP 工具被发现并可调用
```
...
```bash
# 7.1 Telegram 接入 (需要配置 config.json)
# 在 .octos/config.json 中添加 gateway 配置:
# "gateway": {"channels": [{"type": "telegram", "token": "YOUR_BOT_TOKEN"}]}
octos gateway
# 预期: Telegram bot 启动，收到消息时回复
...
| 9.3  | Codegen Handler | 节点用 LLM 生成代码 | Agent 执行 | ☐ |
...
# 9.1 DOT 解析
# 创建 pipeline.dot 文件
cat > pipeline.dot << 'EOF'
digraph Pipeline {
  A -> B;
  B -> C;
}
EOF
octos pipeline run pipeline.dot
# 预期: 正确解析节点 A, B, C

# 9.2 Shell Handler
cat > shell-pipeline.dot << 'EOF'
digraph Pipeline {
  shell -> result;
}
EOF
# 需要配置 shell 节点执行 cargo test
octos pipeline run shell-pipeline.dot
# 预期: cargo test 执行，结果传递
...
| 10.6  | before_tool_call Hook | Hook exit 1 | 工具被阻止 | ☐ |
| 10.7  | after_tool_call Hook | Hook exit 0 | 正常记录 | ☐ |
...
# 10.1 配置优先级
# 全局配置
mkdir -p ~/.config/octos
echo '{"provider": "openai"}' > ~/.config/octos/config.json
# 项目配置
echo '{"provider": "anthropic"}' > .octos/config.json
octos chat --provider gemini -m "test"
# 预期: 使用 gemini (CLI 参数最高优先级)

# 10.2 热加载 prompt
octos chat &
# 修改 .octos/config.json 中的 system_prompt
sleep 5
# 预期: 5 秒内新 prompt 生效

# 10.5 配置解析失败
echo '{invalid json}' > .octos/config.json
octos chat -m "hello"
# 预期: 错误提示 + 保留旧配置

# 10.6 before_tool_call Hook
# 配置 hooks
# "hooks": [{"event": "before_tool_call", "command": "exit 1"}]
octos chat -m "run echo hello"
# 预期: 工具被阻止

# 10.14 Feature flags
cargo build -p octos-cli
octos serve
# 预期: serve 不可用 (需要 --features api)
```
...
```bash
# 11.1 循环检测 长度 1
octos chat -m "Keep running echo hello 10 times"
# 预期: 检测到重复调用，注入警告

# 11.4 迭代上限
octos chat --max-iterations 5 -m "count to 100"
# 预期: 5 轮后停止

# 11.5 Token 预算
# 在 config.json 中设置 "max_tokens": 1000
octos chat -m "write a long story"
# 预期: 达到 1000 tokens 后停止

# 11.6 墙钟超时
# 在 config.json 中设置 "max_timeout": "10s"
octos chat -m "solve a complex math problem"
# 预期: 10 秒后超时停止
